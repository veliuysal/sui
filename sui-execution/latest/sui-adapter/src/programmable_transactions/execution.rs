        ...
        match tx_context_kind {
            ...
            TxContextKind::Mutable | TxContextKind::Immutable => {
                ...
                let tx_context = if is_txctx_native {
                    // TODO(dario): copy the context into the extension
                    let ctx = context.tx_context.clone();
                    let tx_context = context.native_tx_context_mut();
                    tx_context.sender = ctx.sender().into();
                    tx_context.digest = ctx.digest();
                    tx_context.epoch = ctx.epoch();
                    tx_context.epoch_timestamp_ms = ctx.epoch_timestamp_ms();
                    tx_context.ids_created = ctx.ids_created();
